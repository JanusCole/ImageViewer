...
// These fields are used to determine if the cache can satisfy the current image request
    ...
    private ImageRepository(ImageDataSource imageDataSource) {
        this.imageDataSource = imageDataSource;
    }

// There an be only one
    public static ImageRepository getInstance (ImageDataSource imageDataSource) {
        if (INSTANCE == null) {
            synchronized (ImageRepository.class) {
                if (INSTANCE == null) {
                    INSTANCE = new ImageRepository(imageDataSource);
                }
            }
        }
        return INSTANCE;
    ...
    public void getImages(final String searchCriteria, final int requestedPageNumber, final ImagesSearchCallback imagesSearchCallback) {
        ...
        // Search the data source using the passed search criteria and a page number
                imageDataSource.getImages(searchCriteria, requestedPageNumber, new ImageDataSource.RemoteImagesSearchCallback() {
                    @Override
                    public void onImagesFound(ImageSearchResults returnedImages) {

                        // Update the number of available images for this request
                        totalImagesAvailable = returnedImages.getTotalHits();
                        ...
                        for (ImageDescription returnedImage : returnedImages.getHits()) {
                        ...
                        imagesSearchCallback.onImagesFound(returnedImages.getHits());
                    }
